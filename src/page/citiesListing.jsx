...
import { setDataCities } from '../redux/productSlide';
...
export default function CitiesListing() {
  ...
  const fetchBooks = async () => {
    const res = await fetch(`${process.env.REACT_APP_SERVER_DOMIN}/city`);
    const resData = await res.json();
    dispatch(setDataCities(resData));
  };
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_DOMIN}/city/${id}`,
        {
          method: 'DELETE',
          headers: {
  ...
  const handleAddNew = () => navigate('/city');
